...
- This is known as "auto-regressive next word prediction" (actually "token prediction").

- The model predicts which word (token) is likely to follow, given a sequence of words (tokens). 
- Key idea: this simple procedure is followed over and over again, with each new word (token) being added to the sequence of words (tokens) that the model uses to predict the next word (token).
$$ P(w_{w+1} | w_1, w_2, ..., w_t) $$

- The sequence of words is called the __context__; the text generated by the model is dependent on the context.
- The output of the model is a probability distribution over all possible words (tokens). The model then chooses one word (token) from this distribution.
