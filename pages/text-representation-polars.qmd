...
colnames = [str(i) for i in range(1, len(embeddings)+1)]
df = pl.DataFrame(dot_product_matrix, schema=colnames)
df = df.with_columns(pl.Series("embedding_index", range(1, len(embeddings)+1)))
df = df.melt(id_vars="embedding_index", variable_name='embedding_index_2', value_name='similarity')
df = df.cast({"embedding_index_2": pl.Int64})
...
sentences_df = pl.DataFrame(sentences, schema=['Sentences'])
sentences_df = sentences_df.with_columns(pl.Series("Index", range(1, len(embeddings)+1)))
